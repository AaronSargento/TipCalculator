...
// Input: User will enter the subtotal via text field, and the tax and tip via steppers.
...
class FourthViewController: UIViewController {
    ...
    @IBOutlet weak var input2: UIStepper!
    
    @IBOutlet weak var input3: UIStepper!
    ...
    /*
     This function will take the value from each stepper and call calculateTotal() to calculate the running total
     */
    @IBAction func stepper(_ sender: UIStepper) {
        switch sender {
        case input2:
            self.s2 = Double(sender.value) / 100
            taxLabel.text = "\(s2)"
        case input3:
            self.s3 = Double(sender.value) / 100
            tipLabel.text = "\(s3)"
        default:
            break
        }
